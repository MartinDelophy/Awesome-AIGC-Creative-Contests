...
- It separately validates and prunes the opt-in shards listed in `data/opportunities/manifest.json`; these records never enter the generated core outputs.
...
2. `scripts/collect_sources.py` checks `robots.txt` and then makes at most one content request per due source, rejects private-network URLs, limits response size, normalizes tracking URLs, and extracts matching links.
3. New links are deduplicated against `data/contests.json`, every opt-in opportunity shard, and earlier candidates. Existing `reviewing` or `rejected` decisions are preserved across later runs.
...
5. Only then is an AIGC-native record added to `data/contests.json`; AI-compatible or selected general opportunities go to a matching opt-in shard. The next discovery run removes either kind from the candidate queue.
...
## Core and opt-in contracts / 核心与可选数据契约

- `data/contests.json` and `data/schema.json` retain the original core AIGC fields and categories. The generated README tables, RSS, ICS, and WeCom digest read only this file.
- `data/opportunities/manifest.json` is the discovery document for extended datasets. Consumers read it first and fetch only the region shards they need.
- Every manifest entry must keep `default_included: false`. No build or collector may silently merge an optional shard into the core feed.
- `data/opportunities/schema.json` defines the required filtering and provenance fields for optional records. IDs and official URLs must be unique across the core and all shards.
- `scripts/validate_opportunities.py --check` enforces the manifest, record counts, source references, schema boundary, and cross-dataset uniqueness.

This separation lets a client opt into national, local, or global opportunity coverage without forcing unrelated regional data on every user. New high-volume regions may be split into more shards by adding a manifest entry; existing consumers remain unaffected.
...
4. For a core change, commit the regenerated `README.md`, `README.zh-CN.md`, `feed.xml`, and `deadlines.ics`. An optional-only change must leave those core outputs unchanged.
...
The repository uses continuously updated datasets rather than numbered releases. `main` is canonical. Notable structural and policy changes are recorded in `CHANGELOG.md`; current core AIGC facts live in `data/contests.json`, while opt-in extensions are enumerated by `data/opportunities/manifest.json`.
